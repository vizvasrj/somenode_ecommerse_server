...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (imageUrl.includes('storjshare.io')) {
        const imageName = imageUrl.split('storjshare.io/')[1];
        console.log(imageName)
        imageUrl = `https://link.ap1.storjshare.io/raw/${keys.storj.readOnlyApiKey}/${keys.aws.bucketName}/${imageName}`;
      }
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.merchant) {
        ...
        console.log("req.user.merchant", req.user.merchant, "req.user", req.user)
        products = await Product.find({});
        console.log("products", products)
      }
